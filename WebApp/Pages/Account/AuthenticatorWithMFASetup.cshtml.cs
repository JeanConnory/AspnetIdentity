...
namespace WebApp.Pages.Account
{
    ...
    public class AuthenticatorWithMFASetupModel : PageModel
    {
        ...
        public async Task OnGetAsync()
        {
			...
			//this.ViewModel.QRCodeBytes = GenerateQRCodeBytes("my web app", key, user);
		}
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            var user = await _userManager.GetUserAsync(base.User);
            if (await _userManager.VerifyTwoFactorTokenAsync(user, _userManager.Options.Tokens.AuthenticatorTokenProvider, this.ViewModel.SecurityCode))
            {
        ...
        //Não tá funcionando no .NET 6
        //private Byte[] GenerateQRCodeBytes(string provider, string key, string userEmail)
        //{
        //    var qrCodeGenerater = new QRCodeGenerator();
        //    var qrCodeData = qrCodeGenerater.CreateQrCode($"otpauth://totp/{provider}:{userEmail}?secret={key}&issuer={provider}",
